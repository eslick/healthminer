...
(defmacro pushrpl (item list &key (key #'identity) (test #'eq))
  (with-gensyms (lval ival)
    `(let ((,lval ,list)
	   (,ival ,item))
       (aif (member (funcall ,key ,ival) ,lval :key ,key :test ,test)
	    (rplaca it ,ival)
	    (push ,ival ,list))
       ,list)))
...
(defun as-symbol (string)
  "Convert to case insensitive symbol or uppercase for modern modes"
...
(defun get-token (n string &optional rest)
  (if rest 
      (concat-separated-strings #\Space (nthcdr n (extract-words string)))
      (nth (1- n) (extract-words string))))
...
;; XML utilities for lxml format from s-xml library

;; XML utilities							

(defun get-lxml-expressions (xml path)
  (cond ((null path) xml)
	((eq (get-lxml-tag xml) (first path))
	 (if (length=1 path)
	     (list xml)
	     (remove-nulls 
	      (mapcan #'(lambda (expr)
			  (get-lxml-expressions expr (rest path)))
		      (get-lxml-body xml)))))
	(t nil)))
(defun get-lxml-expression (xml path)
  (awhen (get-lxml-expressions xml path)
    (first it)))

(defun get-lxml-body-element (xml tag)
  (find tag (get-lxml-body xml) 
	:test (lambda (val expr)
		(eq val (get-lxml-tag expr)))))

(defun get-lxml-arguments (xml argument &optional path)
  (cond ((null path)
	 (when (lxml-has-arguments-p xml)
	   (list (getf (rest (get-lxml-head xml)) argument))))
	((eq (first path) (get-lxml-tag xml))
	 (if (length=1 path)
	     (get-lxml-arguments xml argument)
	     (remove-nulls 
	      (mapcan #'(lambda (expr) 
			  (get-lxml-arguments expr argument (rest path)))
		      (get-lxml-body xml)))))
	(t nil)))
(defun get-lxml-argument (xml argument &optional path)
  (awhen (get-lxml-arguments xml argument path)
    (first it)))


(defun get-lxml-tag (xml) 
  (cond ((not (listp xml))
	 xml)
	((listp (car xml))
	 (caar xml))
...
(defun lxml-has-arguments-p (xml)
  (and (listp xml) (listp (car xml))))

(defun get-lxml-head (xml) 
  (if (listp xml) (first xml) xml))

(defun get-lxml-body (xml) 
  (when (listp xml) (rest xml)))
